[
//Google Apps
{
"device"="all",
"name"="Google Apps",
"zipname"="GAPPS.zip",
"url"="http://cmw.22aaf3.com/gapps/gapps-gb-20110828-signed.zip",
"installable"="false",
"category"="google",
"description"="Google Apps for OMFGB - 09-29"
},

//Apps
{
"device"="all",
"name"="Wallpapers",
"zipname"="Wallpapers.apk",
"url"="https://market.android.com/details?id=com.t3hh4xx0r.wallpapers",
"market"="true",
"installable"="true",
"category"="applications",
"density"="hdpi",
"description"="Custom T3hh4xx0r wallpapers."
},
{
"device"="all",
"name"="Open File Manager",
"market"="true",
"installable"="true",
"zipname"="OFM.apk",
"url"="market://details?id=org.linuxmotion.filemanager",
"category"="applications",
"density"="all",
"description"="Open File Manager(Beta)"
},
{
"device"="mecha",
"name"="MIUI Camera (w/FFC Support via hack)",
"zipname"="MiuiCamera_v17_TEST1.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=19",
"installable"="true",
"category"="applications",
"density"="hdpi",
"description"="The FFC support requires a simple hack. After installed, switch to FFC (you'll get a black screen), go into Settings and change Picture size to 1280x1024, and FFC works! I do not know who to credit with this. If you know, please notify team t3hh4xx0r or Jaxidian so we can give appropriate credit!"
},

//Themes
{
"device"="all",
"name"="OMFT 1.0.1",
"zipname"="OMFT.apk",
"url"="http://r2doesinc.bitsurge.net/Addons/OMFT.apk",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Custom theme designed for OMFGB and OMGB."
},
{
"device"="all",
"name"="Azure",
"zipname"="AzureHDPI.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=10",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Azure by Arzbhatia is a minimalist gray theme. More info: http://l.jaxidian.org/AzureTheme"
},
{
"device"="all",
"name"="Black Funk",
"zipname"="BlackFunk_hdpi.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=6",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Black Funk by Kfunk is a glassy white on black theme. More info: http://l.jaxidian.org/BlackFunkTheme"
},
{
"device"="all",
"name"="Dark Horse",
"zipname"="darkhorse-hdpi.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=7",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Dark Horse by Kfunk is a glassy blue on black theme. More info: http://l.jaxidian.org/DarkHorseTheme"
},
{
"device"="all",
"name"="EpicBlue N Bold",
"zipname"="EpicBlueCM7-1.0.1_08Sep11.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=20",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="(9/08) EpicBlue N Bold by Stetsonaw is a unique and aggressive blue on black theme. More info: http://l.jaxidian.org/EpicBlueTheme"
},
{
"device"="all",
"name"="Indistinct (w/ batt %)",
"zipname"="Indistinct3WithBatteryPercentage.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=8",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Indistinct (w/ batt %) by Arzbhatia is a simple, low-key black-based theme. More info: http://l.jaxidian.org/IndistinctTheme"
},
{
"device"="all",
"name"="Indistinct (w/o batt %)",
"zipname"="Indistinct3WithoutBatteryPercentage.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=9",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Indistinct (w/o batt %) by Arzbhatia is a simple, low-key black-based theme. More info: http://l.jaxidian.org/IndistinctTheme"
},
{
"device"="all",
"name"="SimpleBLU",
"zipname"="SimpleBLUFINAL2.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=18",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="(8/21) SimpleBLU by bretth18 is a blue Gingerbread theme. More info: http://l.jaxidian.org/SimpleBluTheme"
},
{
"device"="all",
"name"="Synergy v1.3",
"zipname"="Synergy-v1.3.apk",
"url"="http://www.jbthemes.com/bgill55/Synergy_donate_v1.3.apk",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Syngergy by BSthemes, more info: http://www.jbthemes.com/bgill55"
},
{
"device"="all",
"name"="Whiteout",
"zipname"="signedNEG_Whiteout_beta_1.3.9.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=5",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="Whiteout by XsMagical (NewEraGFx) is a white-based theme. More info: http://l.jaxidian.org/WhiteoutTheme"
},
{
"device"="all",
"name"="X-Series (Blue)",
"zipname"="BlueX.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=11",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="X-Series (Blue) by Natemz is a blue on black theme. More info: http://l.jaxidian.org/TheXSeries"
},
{
"device"="all",
"name"="X-Series (Green)",
"zipname"="GreenX.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=12",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="X-Series (Green) by Natemz is a green on black theme. More info: http://l.jaxidian.org/TheXSeries"
},
{
"device"="all",
"name"="X-Series (Orange)",
"zipname"="OrangeX.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=13",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="X-Series (Orange) by Natemz is an orange on black theme. More info: http://l.jaxidian.org/TheXSeries"
},
{
"device"="all",
"name"="X-Series (Red)",
"zipname"="RedX.apk",
"url"="http://files.jaxidian.org/Android/download.php?file=14",
"installable"="true",
"category"="themes",
"density"="hdpi",
"description"="X-Series (Red) by Natemz is a red on black theme. More info: http://l.jaxidian.org/TheXSeries"
},

//Kernels
/////Incredible
{
"device"="inc",
"name"="OMFGBk-sbc 1",
"zipname"="OMFGBk-sbc_1.zip",
"url"="http://r2doesinc.bitsurge.net/Addons/OMFGBk-sbc-1.zip",
"installable"="false",
"description"="SBC kernel for OMFGB",
"category"="kernel"
},

/////Mecha
{
"device"="mecha",
"name"="OMFGB-Drod Cfs",
"zipname"="OMFGB-Drod_Cfs.zip",
"url"="http://r2doesinc.bitsurge.net/Addons/OMFGB-Drod_Cfs.zip",
"installable"="false",
"category"="kernel",
"description"="CFS kernel for OMFGB by Drod"
},
{
"device"="mecha",
"name"="KangBang! CFS v-0.9",
"zipname"="KangBang-CFS-0.9.zip",
"url"="http://crackflashers.com/DRod2169/Thunderbolt/Kernels/KangBang-CFS-0.9.zip",
"installable"="false",
"category"="kernel",
"description"="KangBang! CFS v-0.9 by drod2169. More info: http://l.jaxidian.org/KangBangKernel"
},
{
"device"="mecha",
"name"="KangBang! BFS v-1.0",
"zipname"="KangBang-1.0-BFS.zip",
"url"="http://files.jaxidian.org/Android/download.php?file=4",
"installable"="false",
"category"="kernel",
"description"="KangBang! BFS v-1.0 by drod2169. More info: http://l.jaxidian.org/KangBangKernel"
},
{
"device"="mecha",
"name"="leanKernel v-3.5.12",
"zipname"="imoseyon_leanKernel_v3.5.12AOSP.zip",
"url"="http://files.jaxidian.org/Android/download.php?file=21",
"installable"="false",
"category"="kernel",
"description"="leanKernel v-3.5.12 by imoseyon. More info: http://l.jaxidian.org/LeanKernel"
},
{
"device"="mecha",
"name"="leanKernel v-3.6.0",
"zipname"="imoseyon_leanKernel_v3.6.0AOSP.zip",
"url"="http://files.jaxidian.org/Android/download.php?file=22",
"installable"="false",
"category"="kernel",
"description"="leanKernel v-3.6.0 by imoseyon. More info: http://l.jaxidian.org/LeanKernel"
},
{
"device"="mecha",
"name"="leanKernel v-3.6.0 (184MHz)",
"zipname"="imoseyon_leanKernel_184Mhz_v3.6.0AOSP.zip",
"url"="http://files.jaxidian.org/Android/download.php?file=23",
"installable"="false",
"category"="kernel",
"description"="leanKernel v-3.6.0 (unlocks 184MHz but is unstable and overheats for some) by imoseyon. More info: http://l.jaxidian.org/LeanKernel"
},
{
"device"="mecha",
"name"="Tiamat v1.1.0",
"zipname"="Tiamat_Mecha-v1.1.0.zip",
"url"="http://files.jaxidian.org/Android/download.php?file=15",
"installable"="false",
"category"="kernel",
"description"="Tiamat v1.1.0 by Cayniarb. More info: http://tiamat-dev.com"
},

/////vibrant
{
"device"="vibrantmtd",
"name"="Bali 2.2",
"zipname"="Bali-CM_v2.2.zip",
"url"="http://goo-inside.me/kernels/edt/vibrant/Bali-CM_v2.2.zip",
"installable"="false",
"category"="kernel",
"description"="drhonk's magical kernel. Voodoo sound & color. oc/uv support."
},
{
"device"="vibrantmtd",
"name"="Bali 2.2 w/BLN",
"zipname"="Bali-CM_v2.2-LED.zip",
"url"="http://goo-inside.me/kernels/edt/vibrant/Bali-CM_v2.2-LED.zip",
"installable"="false",
"category"="kernel",
"description"="drhonk's magical kernel. Voodoo sound & color. oc/uv support."
},
{
"device"="vibrantmtd",
"name"="Immortality v1",
"zipname"="Immortality-CM_v1.zip",
"url"="http://goo-inside.me/kernels/edt/vibrant/Immortality-CM_v1.zip",
"installable"="false",
"category"="kernel",
"description"="An alternate custom kernel with OC/UV made my Mr. Apocalypse"
},
{
"device"="vibrantmtd",
"name"="GPS Fix",
"zipname"="Vibrant_CM7_GPSFix_V1.5-CWM-signed.zip",
"url"="http://goo-inside.me/roms/edt/vibrant/Vibrant_CM7_GPSFix_V1.5-CWM-signed.zip",
"installable"="false",
"category"="applications",
"description"="Strictlyrude27's CM7 GPS Fix. Works wonders."
},

///g2x
{
"device"="p999",
"name"="Faux -- DS OC/UV 0.4.1",
"zipname"="P999-DS-Kernel-0.4.1-ext4.zip",
"url"="http://goo-inside.me/kernels/edt/g2x/P999-DS-Kernel-0.4.1-ext4.zip",
"installable"="false",
"category"="kernel",
"description"="DS battery driver. OC/UV by default @ 1.1Ghz. Max @ 1.55GHz."
},
{
"device"="p999",
"name"="Faux -- CM OC/UV 0.4.1",
"zipname"="P999-CM-Kernel-0.4.1-ext4.zip",
"url"="http://goo-inside.me/kernels/edt/g2x/P999-CM-Kernel-0.4.1-ext4.zip",
"installable"="false",
"category"="kernel",
"description"="CM battery driver. OC/UV by default @ 1.1Ghz. Max @ 1.55GHz."
},
{
"device"="p999",
"name"="Faux -- DS 0.4.1",
"zipname"="P999-DS-Kernel-0.4.1-ext4-sv.zip",
"url"="http://goo-inside.me/kernels/edt/g2x/P999-DS-Kernel-0.4.1-ext4-sv.zip",
"installable"="false",
"category"="kernel",
"description"="DS battery driver. Stock voltages, default @ 1.0Ghz. Max @ 1.4GHz."
},
{
"device"="p999",
"name"="Faux -- CM 0.4.1",
"zipname"="P999-CM-Kernel-0.4.1-ext4-sv.zip",
"url"="http://goo-inside.me/kernels/edt/g2x/P999-CM-Kernel-0.4.1-ext4-sv.zip",
"installable"="false",
"category"="kernel",
"description"="DS battery driver. Stock voltages, default @ 1.0Ghz. Max @ 1.4GHz."
}


]

